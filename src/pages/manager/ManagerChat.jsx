...
useEffect(() => {
  const checkForVideoCallNotification = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/notifications/${manager.manager._id}`);
                  ...
                  {messages.map((msg, index) => (
                    ...
                    >
                      ...
                      {msg.sender === sender && (
                        <div className="relative flex-shrink-0 w-8 h-8 rounded-full overflow-hidden">
                          <img
                            src={sender.profileImageUrl ? currentUser.sender : 'https://i.pinimg.com/564x/00/80/ee/0080eeaeaa2f2fba77af3e1efeade565.jpg' }
                            alt="User"
                            className="object-cover w-full h-full"
                          />
                        </div>
